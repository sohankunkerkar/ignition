...
func FetchFromOvfDevice(f *resource.Fetcher, ovfFsTypes []string) (types.Config, report.Report, error) {
	...
	for _, ovfFsType := range ovfFsTypes {
		device, err := execUtil.GetBlockDevices(ovfFsType)
		if err != nil {
		...
		for len(device) > 0 {
			for i, dev := range device {
				// verify that this is a CD-ROM device
				if isCdromPresent(logger, dev) {
					...
					if len(rawConfig) >= 0 {
						...
						logger.Debug("failed to retrieve config: %v", err)
					}
				}
			}
			...
			device, _ = execUtil.GetBlockDevices(ovfFsType)
		}
	}
...
// isCdromPresent verifies if the given config drive is a CD-ROM
// device.
