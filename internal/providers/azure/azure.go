...
func FetchFromOvfDevice(f *resource.Fetcher, ovfFsTypes []string) (types.Config, report.Report, error) {
	...
	for _, ovfFsType := range ovfFsTypes {
		...
		for len(devices) == 0 {
			...
			devices, _ = execUtil.GetBlockDevices(ovfFsType)
		}
		for _, dev := range devices {
			// verify that this is a CD-ROM drive with a UDF
			// volume attached to it. This helps to avoid
			// reading data from an arbitrary block device
			// attached to the VM by the user.
